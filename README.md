...
POST /sii/pedimento-status-changed
```
...
For pedimento notifications, both endpoints (`/sii/pedimento-created` and `/sii/pedimento-status-changed`) emulate the payload currently generated by TraficoWeb's pedimento status flow:
...
- `actionPayload`: pedimento value

You can provide `pedimento` in the request body (or `actionPayload` as fallback).
...
```bash
curl -X POST http://localhost:3000/sii/pedimento-status-changed \
    -H "Content-Type: application/json" \
    -d '{
        "deviceId": "FCM_DEVICE_TOKEN",
        "pedimento": "24  16  1234  0001234",
        "perfilId": "10"
    }'
```
